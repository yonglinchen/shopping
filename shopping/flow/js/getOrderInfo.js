    ...
    apiSendAjax(send_url, service, true, function (status, data) {
        if(status == 0){
            ...
            for(var _j=0; _j<addressList.length; _j++){
                ...
                $("#consignee-list").append('<li class="ui-switchable-panel"><div class="consignee-item '+Default+'"><input type="hidden" value="'+addressList[_j].address_id+'" class="address_id"><span>'+addressList[_j].consignee+' '+addressList[_j].province+'</span><b></b></div><div class="addr-detail"><span class="addr-name">'+addressList[_j].consignee+' </span><span class="addr-info">'+addressList[_j].province+' '+addressList[_j].city+' '+addressList[_j].district+' '+addressList[_j].address+'</span><span class="addr-tel">'+addressList[_j].mobile+'</span></div><div class="op-btns" style="display:none;"><a href="javascript:;" class="ftx-05 setdefault-consignee">设为默认地址</a><a href="javascript:;" class="ftx-05 edit-consignee" onclick="use_NewConsignee()">编辑</a><a href="javascript:;" class="ftx-05 del-consignee">删除</a></div></li>');
            }
    ...
    apiSendAjax(send_url, service, true, function (status, data) {
        if(status == 0){
            ...
            $(".vip_select1").change(function(){
                $(".vip_select2").html("");
                $(".vip_select3").html("");
                $(".vip_select2").append('<option>请选择</option>');
                $(".vip_select3").append('<option>请选择</option>');
...
function addUserAddr(){
    ...
    apiSendAjax(send_url, service, true, function (status, data) {
        if(status == 0){
            ...
            $("#consignee-list").append('<li class="ui-switchable-panel"><div class="consignee-item"><span>'+name+' '+Province2+'</span><b></b></div><div class="addr-detail"><span class="addr-name">'+name+' </span><span class="addr-info">'+Province2+' '+City2+' '+District2+' '+address+'</span><span class="addr-tel">'+tel+'</span></div><div class="op-btns" style="display:none;"><a href="javascript:;" class="ftx-05 setdefault-consignee">设为默认地址</a><a href="javascript:;" class="ftx-05 edit-consignee">编辑</a><a href="javascript:;" class="ftx-05 del-consignee">删除</a></div></li>');
            console.log(data);
        } else {
            console.log(data);
        }
