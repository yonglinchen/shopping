...
//                print_r($string.'   ******    ');
	 ...
	 * @return 签名，本函数不覆盖sign成员变量，如要设置签名需要调用SetSign方法赋值
	 */
	public function MakeSignSpecil()
	{
		//签名步骤一：按字典序排序参数
//                print_r($this->values) ;
//		ksort($this->values);
...
//                print_r($string.'   ******    ');
		//签名步骤三：MD5加密
//                $string='appid=wxf92e622f0985ef8a&noncestr=lOl6mgnPg6u8XQA2&package=Sign=WXPay&partnerid=1263121101&prepayid=wx201512042040236c473ab3a70091120148&timestamp=1449232735&key=master20151201000000000000000000';
		$string = md5($string);
		//签名步骤四：所有字符转为大写
		$result = strtoupper($string);
		return $result;
 ...
 *
 */
class WxPayReplyData extends  WxPayDataBase
{
	/**
	 * 
	 * 设置appId
