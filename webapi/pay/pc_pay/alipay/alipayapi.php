...
/**************************请求参数**************************/
    //支付类型
        ...
        //必填，不能修改
        //服务器异步通知页面路径
        $notify_url = $alipay_config["alipay_notify_url"];//"http://商户网关地址/create_direct_pay_by_user-PHP-UTF-8/notify_url.php";
        //需http://格式的完整路径，不能加?id=123这类自定义参数

        //页面跳转同步通知页面路径
        $return_url = $alipay_config["alipay_return_url"];//"http://www.testlxd.com/alipaydemo/return_url.php";
        ...
        /* $out_trade_no =  $_SESSION['$userbyvipordernum'];*/
//        $out_trade_no = $_POST['WIDout_trade_no'];
        
         //必填
        //订单名称，发送post请求，查询cost、订单名
...
//             $url = '../../view/vip/vip.html?errorinfo='.$ret['msg'];    
//             header("Location:".$url);
        }
...
//        }
        ...
        $out_trade_no = $ret['order_no'];//.'-'.$randnum; //记录订单号+_+随机数
        //付款金额
         $total_fee = $ret['out_data']['fee'] / 100; //支付宝支持：元
         // 日志
        $info1 = 'alipay/alipay.php,operator success,order_no:'.$out_trade_no
            .',totalfee:'.$total_fee;
        ilog(iLOG_INFO, $info1 , __LINE__);
 
        //订单描述
        $body = _PAYSUBJECT_.'，订单为：'.$out_trade_no;//$_POST['WIDbody'];
        //商品展示地址
