...
// | 文件上传类 
// +----------------------------------------------------------------------
// | Author: 陈泳霖 <15001209007@163.com> 
// +----------------------------------------------------------------------
...
class Upload {
    ...
    private function setDriver($driver = null, $config = null){
        ...
        include_once dirname(__FILE__) . (strpos($driver,'\\')? $driver : '/Upload/Driver/'.ucfirst(strtolower($driver))) . ".class.php";
        $classReal = ucfirst(strtolower($driver));
        $this->uploader = new $classReal($config);
        if(!$this->uploader){
            throw new \Exception("不存在上传驱动");
        }
    }
    ...
    private function getSaveName($file) {
        ...
        return $ext == "" ? $savename : $savename . '.' . $ext;
    }
